...
export function useEntities() {
  ...
  const [entityTypes] = useLocalStorage<EntityType[]>(STORAGE_KEYS.ENTITY_TYPES, []);
  const [workflowRecords, setWorkflowRecords] = useLocalStorage<WorkflowRecord[]>(
  ...
  const create = useCallback(
    (data: CreateEntityInput): Entity => {
      const entityType = entityTypes.find((et) => et.id === data.entity_type_id);
      if (!entityType) {
      ...
      const assignedWorkflowIds = entityTypeWorkflows[data.entity_type_id] || [];
      const newWorkflowRecords: WorkflowRecord[] = assignedWorkflowIds.map((workflowId) => {
        ...
        return {
          ...
          current_step: firstStep
            ? { id: firstStep.id, name: firstStep.name, order_index: firstStep.order_index }
            : null,
          status: 'not_started',
          step_submissions: [],
          started_at: null,
    ...
    [entities, entityTypes, entityTypeWorkflows, workflows, setEntities, setWorkflowRecords]
  );
        ...
        prev.map((e) => {
          if (e.id === id) {
            ...
            updated = {
              ...
              parent_id: data.parent_id !== undefined ? data.parent_id : e.parent_id,
              parent: parent
                ? { id: parent.id, code: parent.code, name: parent.name }
                : data.parent_id === null
                ? null
